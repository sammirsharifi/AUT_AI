    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                cost = 1
                corner_visite_condition= state[1]

                # Next state is in a corner
                if nextState in self.corners:
                    index = self.corners.index(nextState)
                    corner_visite_condition=list(corner_visite_condition)
                    corner_visite_condition[index]=True
                    corner_visite_condition=tuple(corner_visite_condition)
 
                successors.append(((nextState, corner_visite_condition), action, cost))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.iterative_deepening_depthFirstSearch(problem,100)
